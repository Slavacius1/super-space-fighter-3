...
class Mode7:
    def __init__(self, app, texture1, texture2, fogMode = 0, scale = 100):

        self.app = app
        self.floor_tex = pg.image.load(texture1).convert()
        self.tex_size = self.floor_tex.get_size()
        self.floor_array = pg.surfarray.array3d(self.floor_tex)

        self.ceil_tex = pg.image.load(texture2).convert()
    ...
    def update(self):
        ...
        self.screen_array = self.render_frame(self.floor_array, self.ceil_array, self.screen_array,
                                              self.tex_size, self.angle, self.pos, self.alt, self.fogMode, self.scale)

    def draw(self, surf):
        pg.surfarray.blit_array(surf, self.screen_array)

    @staticmethod
    @njit(fastmath=True, parallel=True)
    def render_frame(floor_array, ceil_array, screen_array, tex_size, angle, player_pos, alt, fogMode, scale):
